...
export default function OverviewPanel() {
  ...
  return (
    <Container
      className="game-overview-container w-75 position-absolute"
          ...
          <div className="game-table-container table-ul">
            {Array.from({ length: 50 }, (_, i) => i + 1).map((i) => (
              <TableSlot key={i} tableSlotIdx={i}>
                {hand.playerIdx === -1 ? null : tables[
                    (hand.playerIdx + 3) % 4
                  ][i] ? (
                  <TableCard cardId={tables[(hand.playerIdx + 3) % 4][i]} />
                ) : null}
              </TableSlot>
            ))}
          </div>
            ...
            {Array.from({ length: 50 }, (_, i) => i + 1).map((i) => (
              <TableSlot key={i} tableSlotIdx={i}>
                {hand.playerIdx === -1 ? null : tables[
                  ...
                  <TableCard cardId={tables[(hand.playerIdx + 2) % 4][i]} />
                ) : null}
              </TableSlot>
            ))}
          </div>
      ...
      <Row className="d-flex h-50 gap-2">
        <Col className="game-table-wrapper">
          <div className="game-table-container table-bl">
            {Array.from({ length: 50 }, (_, i) => i + 1).map((i) => (
              <TableSlot key={i} tableSlotIdx={i}>
                {hand.playerIdx === -1 ? null : tables[hand.playerIdx][i] ? (
                  <TableCard cardId={tables[hand.playerIdx][i]} />
                ) : null}
              </TableSlot>
            ))}
          </div>
            ...
            {Array.from({ length: 50 }, (_, i) => i + 1).map((i) => (
              <TableSlot key={i} tableSlotIdx={i}>
                {hand.playerIdx === -1 ? null : tables[
                  ...
                  <TableCard cardId={tables[(hand.playerIdx + 1) % 4][i]} />
                ) : null}
              </TableSlot>
            ))}
          </div>
